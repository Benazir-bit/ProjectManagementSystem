...
class RaisedIssues extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fetchingData: true,
  ...
  loadData = () => {
    ...
    axios
      .get(
        `/uspl/api/user/issues/raised/${this.props.match.params.id}?offset=${offset}&limit=${limit}`,
        config
      )
      .then(res => {
        const newJournals = res.data.results;
        console.log("fetching...", ...newJournals, newJournals);
  ...
  render() {
    ...
    var tableData = {
      columns: ["Issue Name", "Task Name", "Raised On", "Status"],
      rows: issue_list
    };
    return (
      <Fragment>
...
export default connect(mapStateToProps, { getTypeIssues })(RaisedIssues);
