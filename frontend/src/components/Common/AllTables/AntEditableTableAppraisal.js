    ...
    return editing ? (
      <Form.Item style={{ margin: 0 }}>
        {form_.getFieldDecorator(dataIndex + "_" + String(record.key), {
          rules: [
            {
              required: RegExp("_comment").test(dataIndex) ? false : true,
...
class AntEditableTable extends Component {
  constructor(props) {
    ...
    //     // width: "44%",
    //     editable: this.props.can_edit ? true : false,
    //     render: (value, row, index) => {
    //       const obj = {
    //         children: (
    //           <div
    ...
    //         ? ""
    //         : `Marks (Out of ${this.props.dataSource.max_mark})`,
    //     dataIndex: `${this.props.dataSource.count_temp}_edited_text`,
    //     // width: "44%",
    //     // editable: false
    //     render: (value, row, index) => {
    //       const obj = {
    ...
    //         props: {}
    //       };
    //       if (this.props.dataSource.edited === true) {
    //         // obj.props.rowSpan = this.state.dataSource;
    //         obj.props.colSpan = 1;
    ...
    //   },
    //   {
    //     title: this.props.dataSource.comment === true ? "Comments" : "Average",
    //     dataIndex: `${this.props.dataSource.count_temp}_comment`,
    //     // width: "44%",
    //     editable: this.props.can_edit ? true : false,
    //     render: (value, row, index) => {
    //       const obj = {
  ...
  handleSave = row => {
    ...
    newData.map((item, i) => {
      avg_mark =
        avg_mark + item[`${this.props.dataSource.count_temp}_parameter_mark`];
      // //////console.log("return item ", item.mark);
      data_new.push({
    ...
    data_return["avg_mark"] = isNaN(avg_mark_fix2)
      ? "Section's All Mark Not Given Yet"
  ...
  render() {
        ...
        render: (value, row, index) => {
          const obj = {
            children: (
              <div
                style={{
                  width: this.props.dataSource.comment ? 350 : 150,
                  textAlign: this.props.dataSource.comment ? "left" : "center",
                  // wordBreak: "break-all",
                  // textAlign: "center",
                  color: "rgba(0, 0, 0, 0.85)",
                  fontWeight: 600
    ...
    const columns = columns_main
      ? columns_main.map(col => {
        if (!col.editable) {
          return col;
        }
        return {
